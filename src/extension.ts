...
export function activate(context: vscode.ExtensionContext) {
  context.subscriptions.push(
    ...
    const quize = selectRandomQuize(); 
	  const selectedAns = await  vscode.window.showInformationMessage(quize.q,  quize.opt1, quize.opt2);
    
    if(selectedAns) {
      if(selectedAns === quize.ans){
        vscode.window.showInformationMessage(`You are correct! the answer is ${quize.ans}`);
      }else{ 
        vscode.window.showErrorMessage(`Wrong the the correct answer is ${quize.ans}`); 
      }
  }
